...
| Telegram Mode A Latest-Done Selection Fix | ✅ **Fixed + manually validated** (batch 0246–0250, 2026-05-14) — `Pick latest done file` was using order-dependent selection (`files[files.length - 1]`), selecting stale task 0232 instead of latest 0245; fix: numeric sort descending by four-digit task ID; node-level validation confirmed task_id 0245; one manual execution: **Telegram 0245 arrived**; first **scheduled post-fix tick pending observation** — Telegram Mode A not declared stable-after-fix yet; no implementer runtime; no secrets | `docs/automation/telegram-mode-a-latest-done-selection-fix.md` |
