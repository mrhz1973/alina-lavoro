...
**Telegram Mode A — stato corrente (batch 0224–0226, 2026-05-14):** **D-0221-A = 3 decided** (batch 0224–0226, 2026-05-14): scelta **cleanup-first path** prima dell'attivazione Schedule Trigger. L'utente ha anche registrato un **intent condizionato di follow-on activation**: dopo che il cleanup ha successo e nessun nuovo rischio emerge, proseguire verso il percorso controllato di attivazione Schedule Trigger senza ri-chiedere la stessa scelta strategica. L'intent **non** attiva di per sé lo Schedule Trigger e **non** autorizza Execute. **Cleanup plan creato:** `docs/automation/telegram-mode-a-pre-schedule-cleanup-plan.md` (task 0225) — due candidate non-blocking: stale `D-0165-A` scope_note in `Build notification payload`; `short_hash` empty mapping in `Store notification state`. Telegram notifier production-like resta **manual-only / inactive**; nessun Schedule Trigger attivato; nessuna notifica Telegram automatica attiva. Duplicate-skip resta **conclusively validated** sull'harness fully-pinned (D-0209-A). D-0217-A resta decided/completed; D-0213-A = 3 resta decided; D-0209-A e D-0206-A restano decided/completed; D-0202-A resta superseded. Queue reader non toccato. No app/deploy/tag/rollback. No provider API. **Prossimo step:** cleanup manuale supervisionato in n8n UI, un passo alla volta — niente Execute, niente Schedule.
