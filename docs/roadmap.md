...
**V2.2.0 deployed** — deploy **`@31`** (task 0390, 2026-05-16); **no-login direct start**: access code gate removed entirely; `requireAccess_` no-op; `loginView` rimosso; app si apre direttamente; `initBackground_()` carica dati dal server in background; tag **`v2.2.0-stable`** pending test utente (task 0391). **V2.1.1 stabile** — deploy **`@30`** (task 0365, 2026-05-16); tag **`v2.1.1-stable`** creato (task 0367, 2026-05-16); test manuale OK — "tutto ok 2.1.1" (task 0366, 2026-05-16); quick resume persistence fix: `tryQuickResumeFromCache_` ora accetta `savedCode + cache exists` senza richiedere `c.accessCode === savedCode`; `state.accessCode = savedCode` + `saveCache()` prima di `showApp()`; postura: maintenance-mode, monitor only. **V2.1.0 post-deploy test FAILED** (task 0362, 2026-05-16): app chiedeva codice ad ogni riapertura; tag `v2.1.0-stable` non creato. **V2.1.0 deployed** — deploy **`@29`** (task 0361, 2026-05-16); **analytics/charts**: collapsible card su Mesi, mini dashboard annuale, barre ore/giorni, confronto stipendi. **V2.0.1 stabile** — deploy **`@28`** (task 0327, 2026-05-15); **quick resume from cache**: se LS_ACCESS + cache valida → app visibile subito, server login in background; tag **`v2.0.1-stable`** (task 0331); **test manuale OK** — user "tutto ok 2.0.1" (task 0328, 2026-05-15); **MVP mature / maintenance-ready** (task 0352, batch 0328–0352). **Produzione precedente stabile: V2.0.0 `@26`** (tag `v2.0.0-stable`, task 0323–0324). **V2.0.0** — **stabile 2026-05-15** su **`main`**; include: refactor **DOM API** Dettaglio mese (task 0316), **sticky header mobile** Indietro (task 0317), validazione statica (task 0318), **test fisico OK** Redmi 9C NFC utente (task 0319), bump versione **2.0.0** (task 0322); etichetta V1.9.3 saltata (task 0322); deploy clasp **@26** (stesso deployment ID di @24, description "V2.0.0", task 0323, 2026-05-15); **test manuale /exec OK** post-deploy (task 0324, 2026-05-15); **tag stabile `v2.0.0-stable`** creato (task 0324). **V1.9.2** — release **2026-05-10** su **`main`** (**Dettaglio mese** più **visivo**: riepilogo **metriche**, **card** giorno, **barre proporzionali** ore; solo frontend); **in produzione** Apps Script **`@24`** (`package.json` / `APP_VERSION` **1.9.2**); tag **`v1.9.2-stable`**; snapshot **`gas-current/`**; sessione `docs/sessions/2026-05-10-v192-month-detail-visual-refresh-deploy.md`. **Test manuale utente su `/exec` @24:** **OK** (2026-05-10; incluso **Redmi 9C NFC**). Include **V1.9.1** (**Stipendio** nascosto sul mese corrente in **Mesi**, deploy **`@23`**, test **OK**) e **V1.9.0** (Dettaglio mese MVP, **`@22`**). **Nota uso:** sul telefono va il link Web App **`/exec`**; il Google Sheet è solo database/amministrazione. **Limitazione nota:** il banner Google «Questa applicazione è stata creata da un utente di Google Apps Script» è **esterno** all’app (piattaforma GAS), non bug UI — chiudibile con X ma può riapparire; vedi sessione. **Nota roadmap:** ottimizzazioni future Mesi/Home/Note restano evolutive. **Precedente V1.9.1:** deploy **`@23`**, tag **`v1.9.1-stable`**. **Precedente V1.9.0:** deploy **`@22`**, tag **`v1.9.0-stable`**. Workflow: **`main` operativo**, **`dev` legacy** — `docs/sessions/2026-05-03-main-only-workflow.md`. Tag storici: **`v1.9.1-stable`**, **`v1.9.0-stable`**, **`v1.8.10-stable`**, … **`v1.5-stable`**.
