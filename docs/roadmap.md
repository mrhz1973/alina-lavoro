...
**Telegram Mode A — stato corrente (batch 0182–0184, 2026-05-13):** notifica manuale validata (test OK, task 0170). D-0173-A = 3 decided (schedule deferred). D-0180-A = 1 decided (task 0182): idempotency/state-store runtime gate open. Runtime UI handoff created (task 0183): `docs/automation/telegram-idempotency-runtime-ui-handoff.md`. **Next step:** runtime inspection only — open `TEST - Alina task completion Telegram notifier`, confirm inactive/no Schedule Trigger, check Data Store/Data Table node availability, then stop and report before adding any nodes. If Data Store available: future step-by-step idempotency implementation per design and checklist. If Data Store unavailable: static data fallback design path. Schedule activation remains a separate later gate after idempotency is implemented and validated. Workflow inactive. No Schedule Trigger. No token/chat id in repo.
