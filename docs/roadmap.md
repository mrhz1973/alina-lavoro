...
**Telegram Mode A — stato corrente (batch 0188–0190, 2026-05-14):** idempotency/state-store implementato dall'utente (task 0185). Data Table `alina_telegram_notifier_state` creato con tutte le colonne. Node chain completa. Un test send/write riuscito. **D-0187-A = 1 deciso** — gate validazione duplicate-skip aperto per esattamente una run manuale. Runtime handoff creato: `docs/automation/telegram-duplicate-skip-validation-runtime-handoff.md`. **Prossimo step:** una run manuale di validazione duplicate-skip (atteso: ramo false, nessun Telegram send, nessuna nuova riga Data Table). Dopo validazione riuscita: gate schedule activation separato. Workflow manual-only / inactive. No Schedule Trigger. No token/chat id in repo. Queue reader non toccato.
