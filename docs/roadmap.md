...
**Telegram Mode A — stato corrente (batch 0185–0187, 2026-05-14):** idempotency/state-store implementato dall'utente. Data Table `alina_telegram_notifier_state` creato con tutte le colonne. Node chain completa implementata (Build idempotency key → Load notification state → Normalize notification state → Decide send or skip → Store notification state). Un test send/write riuscito. **D-0187-A pending:** gate per validazione duplicate-skip (secondo run manuale, ramo false atteso, nessun Telegram send, nessuna nuova riga Data Table). Dopo validazione duplicate-skip: gate separato per schedule activation. Workflow manual-only / inactive. No Schedule Trigger. No token/chat id in repo. Queue reader non toccato.
