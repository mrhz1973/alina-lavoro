...
**Telegram Mode A — stato corrente (batch 0191–0193, 2026-05-14):** idempotency/state-store implementato dall'utente (task 0185). Data Table `alina_telegram_notifier_state` creato con tutte le colonne. Node chain completa. Un test send/write riuscito. **D-0187-A = 1 deciso e consumato** (batch 0188–0190, risultato INCONCLUSIVE batch 0191–0193): run validazione ha usato task 0190 come latest done file, generando nuova chiave idempotency; workflow ha correttamente eseguito send/store per nuova chiave; logica duplicate-skip non testata. **D-0193-A pending** per retry contro stessa chiave 0190. Documentazione aggiornata per chiarire requisito same-key. **Prossimo step:** decisione utente su D-0193-A (Opzione 1: autorizza retry, Opzione 2: non riprovare). Schedule activation rimane gate separato. Workflow manual-only / inactive. No Schedule Trigger. No token/chat id in repo. Queue reader non toccato.
