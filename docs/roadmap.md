...
**Telegram Mode A — stato corrente (batch 0177–0181, 2026-05-13):** notifica manuale validata (test OK, task 0170). D-0171-A = 3 deferred schedule. D-0173-A = 3 decided (task 0177): schedule activation deferred; implement idempotency/state-store first. Idempotency/state-store design created (task 0178): `docs/automation/telegram-notifier-idempotency-state-store-implementation-design.md`. Implementation checklist created (task 0179): `docs/automation/telegram-notifier-idempotency-implementation-checklist.md`. D-0180-A **pending** — idempotency/state-store runtime implementation gate (task 0180). Next: resolve D-0180-A explicitly. If D-0180-A = 1, future work is n8n idempotency runtime implementation only (no Schedule Trigger). Schedule activation remains a later separate gate after idempotency is implemented and validated. Workflow inactive. No Schedule Trigger. No token/chat id in repo.
