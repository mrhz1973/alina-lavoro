...
**Telegram Mode A — stato corrente (batch 0194–0198, 2026-05-14):** idempotency/state-store implementato dall'utente (task 0185). Data Table `alina_telegram_notifier_state` creato con tutte le colonne. **D-0187-A = 1** (batch 0188–0190) e **D-0193-A = 1** (batch 0194–0198) entrambi decisi/consumati con esiti INCONCLUSIVE per latest-done drift: in entrambe le run il workflow ha selezionato un done file più recente di quello atteso, generando una nuova chiave e instradando TRUE/send/store (Telegram arrivato). Classificazione: probabili new-key send (categoria b), NON bug confermato di idempotency (categoria c). Root cause documentata: `docs/tasks/done/0195-document-latest-done-drift-root-cause.md`. **Design pinned-file** creato: `docs/automation/telegram-pinned-file-duplicate-skip-validation-design.md`. **D-0197-A pending** per esattamente una run pinned-file. Schedule activation rimane bloccato finché una run pinned-file di categoria (a) non viene registrata. Workflow manual-only / inactive. No Schedule Trigger. No token/chat id in repo. Queue reader non toccato. **Prossimo step:** decisione utente su D-0197-A (1: autorizza pinned-file run; 2: non eseguire; 3: rinvia/raffina design).
