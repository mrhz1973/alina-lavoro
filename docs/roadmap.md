...
**Telegram Mode A — stato corrente (batch 0199–0203, 2026-05-14):** idempotency/state-store implementato dall'utente (task 0185). Data Table `alina_telegram_notifier_state` creato con tutte le colonne. **D-0187-A**, **D-0193-A**, **D-0197-A** tutti consumati: D-0187-A/D-0193-A inconclusive per latest-done drift; D-0197-A = 1 NOT SUCCESSFUL per partial pinning / dynamic reference leakage downstream: la run pinned ha instradato TRUE / Telegram arrivato / Store ha scritto task 0198 (non 0193) / `Load notification state` non ha restituito i campi della riga esistente 0193. Root cause: l'override parziale (Set node tra Build idempotency key e Load notification state) non impedisce ai nodi downstream di referenziare output upstream dinamici per nome. NON è bug confermato di idempotency — il test harness è fallito, non necessariamente la logica. **Design fully-pinned harness** creato: `docs/automation/telegram-fully-pinned-validation-harness-design.md`. **D-0202-A pending** per inspection/repair controllato del workflow TEST-only pinned (senza Execute). Dopo inspection/repair: gate separato per Execute, poi (se successo categoria a) gate schedule activation. Workflow manual-only / inactive. No Schedule Trigger. No token/chat id in repo. Queue reader non toccato. **Prossimo step:** decisione utente su D-0202-A (1: autorizza inspection/repair senza Execute; 2: non riparare ora; 3: rinvia/raffina design).
