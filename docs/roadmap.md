...
**Telegram Mode A — stato corrente (tasks 0170–0173, 2026-05-13):** notifica manuale validata (test OK, task 0170). Schedule activation deferred: D-0171-A = 3. Runbook/idempotency hardening doc created: `docs/automation/telegram-notifier-runbook-idempotency-hardening.md` (task 0172). Pending Decision Packet D-0173-A per schedule activation (task 0173). Next before schedule: implementare idempotency/state-store in n8n, poi risolvere D-0173-A. Workflow `TEST - Alina task completion Telegram notifier` remains inactive. No Schedule Trigger. No token/chat id in repo. Auto-follow recommendation policy: future/optional consideration, not active (see `docs/automation/auto-aggio-design.md`). No provider API LLM. No new billing.
