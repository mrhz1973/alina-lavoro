...
1. Prossimo passo operativo: scelta architetturale per sbloccare task 0114 (Opzione A: investigare n8n per rendere pubblicabile il queue reader; Opzione B: Schedule Trigger direttamente nel queue reader; Opzione C: alternativa sicura). Nessuna modifica runtime senza gate manuale e approvazione orchestratore. Runner documentale automatico (Claude Code CLI / Cursor CLI) resta fuori scope e futuro.
