...
**Status:** active reference document — **updated task 0157 (2026-05-13): D-0157-A pending in INBOX for candidate D Telegram Mode A notifier gate**
...
| D | **Telegram notifier Mode A** (bot creation, token in n8n vault, first message) | Telegram runtime | **Decision Packet pending** — D-0157-A added to `docs/INBOX.md` `## Pending` (task 0157, 2026-05-13); gate is NOT yet open; requires explicit user response; orchestrator recommendation: Option 2 (defer); Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response. Source: `docs/automation/telegram-browser-bridge-trigger-coordination-design.md` Mode A. | Yes |
