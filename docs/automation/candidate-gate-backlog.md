...
**Status:** active reference document — **updated task 0164 (2026-05-13): manual Telegram credential prerequisite complete by user report; bot exists; `telegram_alina_notifier` credential in n8n tested OK; chat id saved privately; no token/chat id in repo; n8n workflow creation remains separately gated**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Credential prerequisite complete by user report (task 0164, 2026-05-13)** — `D-0157-A = 1` (task 0159) + `D-0163-A = 1` (task 0163) decided; user reported: bot exists, `telegram_alina_notifier` credential in n8n tested OK, chat id saved privately; no token/chat id in repo; no n8n workflow yet; no Telegram message yet; MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`; credential guide: `docs/automation/telegram-mode-a-credential-prerequisite-guide.md`; Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. Next runtime phase: n8n notifier workflow creation task TBD (requires explicit gate + PRIORITÀ 0 step-by-step supervision). | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — credential prerequisite complete by user report (task 0164, 2026-05-13).** `D-0157-A = 1` (task 0159) + `D-0163-A = 1` (task 0163) decided. User reported: bot exists, `telegram_alina_notifier` credential in n8n tested OK, chat id saved privately. No token/chat id in repo. No n8n workflow yet. No Telegram message yet. No provider API LLM. No new LLM billing. Next runtime phase: n8n notifier workflow creation task TBD (requires explicit gate + PRIORITÀ 0). Telegram must not answer INBOX or execute project actions.
