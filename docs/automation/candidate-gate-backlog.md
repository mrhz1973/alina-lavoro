...
**Status:** active reference document — **updated task 0169 (2026-05-13): `D-0169-A = 1` recorded; exactly one manual Telegram test message gate open; workflow must remain inactive; no Schedule Trigger; no automatic notifications; next gated phase after test: schedule activation decision (only if test succeeds)**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Single manual test message gate open — `D-0169-A = 1` (task 0169, 2026-05-13).** Credential prerequisite complete (task 0164). Workflow skeleton created (task 0166). Telegram node added by user report (task 0168): Manual Trigger → List done files → Pick latest done file → Get done file → Build notification payload → Telegram; credential by name only (`telegram_alina_notifier`); text `{{ $json.telegram_message }}`; chat id in n8n UI only. `D-0169-A = 1`: exactly one future manual test message allowed under user supervision; workflow must remain inactive; no Schedule Trigger; no automatic notifications. No token/chat id in repo. Queue reader untouched. Next gated phase after test: schedule activation (only if test succeeds). MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`. Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — single manual test message gate open (task 0169, 2026-05-13).** `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` + `D-0169-A = 1` all decided. Credential prerequisite complete (task 0164). Workflow skeleton created (task 0166). Telegram node added by user report (task 0168). `D-0169-A = 1`: exactly one future manual test message allowed under user supervision; workflow must remain inactive; no Schedule Trigger; no automatic notifications; retry requires separate gate. No token/chat id in repo. Existing queue reader untouched. Next gated phase after test: schedule activation decision (only if test succeeds). Schedule activation requires separate future gate. No provider API LLM. No new billing. Telegram must not answer INBOX or execute project actions.
