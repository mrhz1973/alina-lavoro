...
**Status:** active reference document — **updated task 0165 (2026-05-13): `D-0165-A = 1` recorded; n8n notifier workflow creation gate open; scope: create workflow only, no test message, no schedule; next runtime phase: user-supervised n8n UI workflow creation under PRIORITÀ 0; test message remains separately gated**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Workflow creation gate open — `D-0165-A = 1` (task 0165, 2026-05-13).** Credential prerequisite complete (task 0164): bot exists, `telegram_alina_notifier` credential in n8n tested OK, chat id saved privately; no token/chat id in repo. Workflow creation gate: create `TEST - Alina task completion Telegram notifier` in n8n UI step-by-step under PRIORITÀ 0; credential by name only; workflow must remain manual-safe. No test message sent. No Schedule Trigger enabled. Test message: separate future gate. Schedule activation: separate future gate. MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`; credential guide: `docs/automation/telegram-mode-a-credential-prerequisite-guide.md`. Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — workflow creation gate open (task 0165, 2026-05-13).** `D-0157-A = 1` (task 0159) + `D-0163-A = 1` (task 0163) + `D-0165-A = 1` (task 0165) all decided. Credential prerequisite complete (task 0164): bot exists, `telegram_alina_notifier` credential in n8n tested OK, chat id saved privately. No token/chat id in repo. Next runtime phase: user-supervised n8n UI creation of `TEST - Alina task completion Telegram notifier` under PRIORITÀ 0 — one step at a time; no test message; no Schedule Trigger. Test message requires separate future gate. Schedule activation requires separate future gate. No provider API LLM. No new billing. Telegram must not answer INBOX or execute project actions.
