...
**Status:** active reference document — **updated task 0162 (2026-05-13): credential prerequisite guide created (`docs/automation/telegram-mode-a-credential-prerequisite-guide.md`); docs-only; no runtime; no bot/token/config; gate remains open for future narrow implementation only; n8n workflow creation requires separate future gated task**
...
**Candidate D (Telegram Mode A completion notification MVP) — gate open; MVP scaffolding + credential guide complete (tasks 0161–0162, 2026-05-13).** `D-0157-A = 1` decided (task 0159). MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`. Credential guide: `docs/automation/telegram-mode-a-credential-prerequisite-guide.md`. Authorized future narrow implementation: notification-only, task id/slug, commit hash, optional INBOX pending count, instruction "scrivi aggio per post-check". No Telegram implemented. No bot/token/config created. No provider API LLM. No new LLM billing. Next runtime phase: 0163 n8n workflow creation (user-supervised, PRIORITÀ 0). Telegram must not answer INBOX or execute project actions.
