...
|---|-----------|----------|-------|----------------|---------------------|
| A | **Browser Bridge dry-run** (local script writes to a local test file, no browser) | local runtime | **recommended next** | Narrow scope, reversible, no browser, no ChatGPT write, no API key, no billing, validates the trigger pipeline. Safest first runtime-like step. Source: `docs/automation/local-browser-bridge-preflight-design.md` Section 6 MVP path. | Yes |
