...
**Status:** active reference document — **updated batch 0224–0226 (2026-05-14):** **D-0221-A = 3 decided** — cleanup-first path selected; user conditional follow-on activation intent recorded (after cleanup succeeds and no new risk is found, proceed toward controlled Schedule Trigger activation without re-asking the strategic choice — does NOT activate by itself). Cleanup plan: `docs/automation/telegram-mode-a-pre-schedule-cleanup-plan.md` (two non-blocking candidates: stale D-0165-A scope_note + short_hash empty mapping). The production-like Telegram notifier remains manual-only/inactive; no Schedule Trigger is active; no automatic Telegram notification is active. D-0217-A = 1 remains decided/completed. D-0213-A = 3 remains decided. Duplicate-skip remains **conclusively validated** on fully-pinned harness from D-0209-A. No token/chat id in repo; no real secrets in templates. Previous batch status (0219–0223) preserved below as historical reference.
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **D-0221-A = 3 decided (2026-05-14, batch 0224–0226) — cleanup-first path selected; user conditional follow-on activation intent recorded (intent itself does NOT activate Schedule Trigger and does NOT authorize Execute). Cleanup plan: `docs/automation/telegram-mode-a-pre-schedule-cleanup-plan.md`. Next manual work = supervised n8n UI cleanup of stale D-0165-A scope_note in Build notification payload + short_hash empty mapping in Store notification state, one step at a time, no Execute, no Schedule. After successful cleanup with no new risk, proceed toward controlled Schedule Trigger activation per supervision checklist without re-asking the strategic choice.** Prior gates: `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` + `D-0169-A = 1` + `D-0171-A = 3` + `D-0173-A = 3` + `D-0180-A = 1` + `D-0187-A = 1` + `D-0193-A = 1` + `D-0197-A = 1` + `D-0202-A superseded` + `D-0206-A = 1` + `D-0209-A = 1` + `D-0213-A = 3` + `D-0217-A = 1` + `D-0221-A = 3`. Workflow manual-only / inactive. Queue reader untouched. No Schedule Trigger. No token/chat id in repo. Telegram must not answer INBOX. Duplicate-skip conclusively validated from D-0209-A. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — D-0221-A = 3 decided (2026-05-14, batch 0224–0226): cleanup-first path selected.** All prior gates decided: `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` + `D-0169-A = 1` + `D-0171-A = 3` + `D-0173-A = 3` + `D-0180-A = 1` + `D-0187-A = 1` + `D-0193-A = 1` + `D-0197-A = 1` + `D-0202-A superseded` + `D-0206-A = 1` + `D-0209-A = 1` + `D-0213-A = 3` + `D-0217-A = 1` + `D-0221-A = 3`. Cleanup plan: `docs/automation/telegram-mode-a-pre-schedule-cleanup-plan.md`. User conditional follow-on activation intent recorded (does NOT activate; does NOT authorize Execute). Next supervised manual work = n8n UI cleanup of stale D-0165-A scope_note and short_hash empty mapping, one step at a time, no Execute, no Schedule. Supervision checklist for post-cleanup activation: `docs/automation/telegram-mode-a-schedule-activation-supervision-checklist.md`. Workflow remains manual-only / inactive. No Schedule Trigger active. No token/chat id in repo. Queue reader untouched. No provider API LLM. No new billing. Telegram must not answer INBOX.
