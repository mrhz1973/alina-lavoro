...
**Status:** active reference document — **updated task 0163 (2026-05-13): `D-0163-A = 1` recorded; manual Telegram credential prerequisite gate open; no runtime; no bot/token/chat id; n8n workflow creation requires separate future explicit gate**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Gate open; MVP scaffolding + credential guide complete; D-0163-A = 1 decided (tasks 0161–0163, 2026-05-13)** — `D-0157-A = 1` decided (task 0159); `D-0163-A = 1` decided (task 0163): manual Telegram credential prerequisite gate open; MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`; credential guide: `docs/automation/telegram-mode-a-credential-prerequisite-guide.md`; authorized future narrow MVP: notification-only, task id/slug, commit hash, optional INBOX pending count, instruction "scrivi aggio per post-check"; no Telegram implemented; no bot/token/config created; no provider API LLM; no new LLM billing; Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. Next runtime phase: future n8n workflow creation task (number TBD after credential prerequisite; requires explicit step-by-step user supervision, PRIORITÀ 0). | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — gate open; MVP scaffolding + credential guide complete; D-0163-A = 1 decided (tasks 0161–0163, 2026-05-13).** `D-0157-A = 1` (task 0159) + `D-0163-A = 1` (task 0163) decided. Manual Telegram credential prerequisite gate open: user may create bot via BotFather, store token/chat id only in n8n credential vault `telegram_alina_notifier`. MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`. Credential guide: `docs/automation/telegram-mode-a-credential-prerequisite-guide.md`. No Telegram implemented. No bot/token/config created. No provider API LLM. No new LLM billing. Next runtime phase: future n8n workflow creation task (number TBD after credential prerequisite; user-supervised, PRIORITÀ 0). Telegram must not answer INBOX or execute project actions.
