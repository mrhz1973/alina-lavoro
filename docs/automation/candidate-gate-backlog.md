...
**Status:** active reference document — **updated task 0168 (2026-05-13): Telegram node added by user report; workflow saved; credential by name only; chat id in n8n UI only, not in repo; no test message; no Execute/Test; no schedule; next gated phase: single manual Telegram test message decision**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Telegram node added by user report (task 0168, 2026-05-13).** Credential prerequisite complete (task 0164). Workflow skeleton created by user report (task 0166). Telegram node added and saved (task 0168): node chain now Manual Trigger → List done files → Pick latest done file → Get done file → Build notification payload → Telegram; credential by name only (`telegram_alina_notifier`); text `{{ $json.telegram_message }}`; chat id in n8n UI only, not in repo. No test message. No Execute/Test. No Schedule Trigger. No token/chat id in repo. Queue reader untouched. Next gated phase: single manual Telegram test message — requires separate explicit gate. MVP design: `docs/automation/telegram-mode-a-completion-notification-mvp.md`. Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — Telegram node added by user report (task 0168, 2026-05-13).** `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` all decided. Credential prerequisite complete (task 0164). Workflow skeleton created by user report (task 0166). Telegram node added and saved by user report (task 0168): full node chain now includes Telegram node at end; credential by name only; text `{{ $json.telegram_message }}`; chat id in n8n UI only, not in repo. No test message. No Execute/Test. No Schedule Trigger. No token/chat id in repo. Existing queue reader untouched. Next gated phase: single manual test message — requires separate explicit gate. Schedule activation requires separate future gate. No provider API LLM. No new billing. Telegram must not answer INBOX or execute project actions.
