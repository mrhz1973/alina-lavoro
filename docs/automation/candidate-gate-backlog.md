...
**Candidate D (Telegram Mode A completion notification MVP) — D-0171-A = 3 deferred schedule activation (task 0171, 2026-05-13). Runbook/idempotency hardening doc created (task 0172). D-0173-A pending Decision Packet for schedule activation (task 0173).** All prior gates decided: `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` + `D-0169-A = 1`. Manual test succeeded by user report (task 0170). Workflow remains inactive / not automatic. No Schedule Trigger. No token/chat id in repo. Existing queue reader untouched. Runbook: `docs/automation/telegram-notifier-runbook-idempotency-hardening.md`. Next gated phase: resolve D-0173-A; implement idempotency/state-store in n8n before any Schedule Trigger. No provider API LLM. No new billing. Telegram must not answer INBOX or execute project actions.
