...
**Status:** active reference document — **updated task 0158 (2026-05-13): D-0157-A revised in-place in INBOX — scope updated to Telegram Mode A completion notification MVP; recommendation changed to Option 1; D-0157-A still pending**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Decision Packet pending** — D-0157-A revised in-place in `docs/INBOX.md` `## Pending` (task 0158, 2026-05-13); scope: completion notification MVP only — task id/slug, commit hash, optional INBOX pending count, instruction "scrivi aggio per post-check"; orchestrator recommendation: Option 1 (open MVP gate); gate is NOT yet open; requires explicit user response; Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use provider API LLM; must not introduce new LLM billing. Source: `docs/automation/telegram-browser-bridge-trigger-coordination-design.md` Mode A. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — Decision Packet D-0157-A revised in-place (task 0158, 2026-05-13).** Scope narrowed to completion notification MVP: task id/slug, commit hash, optional INBOX pending count, instruction "scrivi aggio per post-check". Recommendation updated to Option 1. D-0157-A remains pending in `docs/INBOX.md` — no user response recorded; gate is NOT open; no Telegram bot/token/config created; no provider API LLM; no new LLM billing. Awaiting explicit user response.
