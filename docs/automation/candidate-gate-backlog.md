...
**Status:** active reference document — **updated batch 0215–0218 (2026-05-14):** **D-0213-A = 3 decided/applied** — schedule activation **deferred**; design-first / template-first path opened in `docs/automation/telegram-mode-a-schedule-activation-design-first-path.md` (task 0216). **D-0217-A pending** (task 0217) — narrower gate authorizing **only** a controlled n8n UI readiness inspection of the intended Telegram Mode A notifier workflow (no Execute, no Schedule Trigger, no Telegram send, no workflow import/export; queue reader untouched). Duplicate-skip remains **conclusively validated** on fully-pinned harness from D-0209-A. D-0209-A and D-0206-A remain decided/completed. D-0202-A remains **superseded** by D-0206-A. The production-like Telegram notifier remains manual-only/inactive; no Schedule Trigger is active; no automatic Telegram notification is active. Schedule activation remains blocked until a future activation gate (separate from D-0217-A) is explicitly decided. No token/chat id in repo; no real secrets in templates. Previous batch status (0211–0214) preserved below as historical reference.
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Idempotency/state-store implemented by user (2026-05-14, task 0185). Data Table `alina_telegram_notifier_state` created. One send/write test succeeded. D-0187-A, D-0193-A, D-0197-A all consumed: D-0187-A/D-0193-A inconclusive (latest-done drift), D-0197-A not successful (partial pinning / downstream dynamic reference leakage). NOT a confirmed idempotency bug — harness failure prevents conclusions. Fully-pinned harness design: `docs/automation/telegram-fully-pinned-validation-harness-design.md`. D-0202-A pending — controlled inspection/repair of TEST-only pinned workflow, no Execute. After inspection/repair: a separate Execute gate, then (if category (a) success) schedule activation gate. No Schedule Trigger. No token/chat id in repo.** Prior gates: `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` + `D-0169-A = 1` + `D-0171-A = 3` + `D-0173-A = 3` + `D-0180-A = 1` + `D-0187-A = 1` + `D-0193-A = 1` + `D-0197-A = 1`. Workflow manual-only / inactive. Queue reader untouched. Fully-pinned harness design: `docs/automation/telegram-fully-pinned-validation-harness-design.md`. Prior (partial) pinned-file design: `docs/automation/telegram-pinned-file-duplicate-skip-validation-design.md`. Runbook: `docs/automation/telegram-notifier-runbook-idempotency-hardening.md`. Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — idempotency/state-store implemented (2026-05-14, task 0185); D-0187-A = 1 decided (batch 0188–0190).** All prior gates decided: `D-0157-A = 1` + `D-0163-A = 1` + `D-0165-A = 1` + `D-0167-A = 1` + `D-0169-A = 1` + `D-0171-A = 3` + `D-0173-A = 3` + `D-0180-A = 1` + `D-0187-A = 1`. Data Table `alina_telegram_notifier_state` created. Full idempotency node chain implemented. One send/write test succeeded. **Duplicate-skip validation gate open** — exactly one manual run authorized. Runtime handoff: `docs/automation/telegram-duplicate-skip-validation-runtime-handoff.md`. Next: perform one duplicate-skip validation run (expected: false branch, no Telegram send, no new Data Table row). After duplicate-skip validated: schedule activation gate (separate future DP). Workflow remains manual-only / inactive. No Schedule Trigger. No token/chat id in repo. Queue reader untouched. No provider API LLM. No new billing. Telegram must not answer INBOX or execute project actions.
