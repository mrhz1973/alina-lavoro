...
**Status:** active reference document — **updated task 0161 (2026-05-13): docs-only scaffolding created for Telegram Mode A completion notification MVP (`docs/automation/telegram-mode-a-completion-notification-mvp.md`); D-0157-A = 1 decided (task 0159); gate open for future narrow implementation only; no Telegram implemented; no bot/token/config; no provider API LLM; no new LLM billing**
...
| D | **Telegram notifier Mode A — completion notification MVP** (bot creation, token in n8n vault, task completion notification) | Telegram runtime | **Gate open; docs-only scaffolding created (task 0161, 2026-05-13)** — `D-0157-A = 1` decided (task 0159, 2026-05-13); MVP design pinned in `docs/automation/telegram-mode-a-completion-notification-mvp.md`; authorized future narrow MVP: notification-only, task id/slug, commit hash, optional INBOX pending count, instruction "scrivi aggio per post-check"; no Telegram implemented; no bot/token/config created; no provider API LLM; no new LLM billing; Telegram must not answer INBOX; must not write any `D-NNNN-X = N` response; must not use Browser Bridge / Ollama / Cursor; must not touch app/deploy/tag/rollback. Future phases: 0162 credential prerequisite → 0163 n8n workflow creation → 0164 validation → 0165 schedule activation. Source: `docs/automation/telegram-mode-a-completion-notification-mvp.md`. | Yes |
...
**Candidate D (Telegram Mode A completion notification MVP) — gate open; docs-only scaffolding complete (task 0161, 2026-05-13).** `D-0157-A = 1` decided (task 0159). MVP design pinned in `docs/automation/telegram-mode-a-completion-notification-mvp.md`. Authorized future narrow implementation: notification-only, task id/slug, commit hash, optional INBOX pending count, instruction "scrivi aggio per post-check". No Telegram implemented. No bot/token/config created. No provider API LLM. No new LLM billing. Future phases: 0162 credential prerequisite → 0163 n8n workflow creation → 0164 validation → 0165 schedule activation. Telegram must not answer INBOX or execute project actions.
