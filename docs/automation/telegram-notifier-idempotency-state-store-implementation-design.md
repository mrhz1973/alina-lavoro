...
**Retry outcome (batch 0194–0198, 2026-05-14):** D-0193-A applied (= 1) per the user's prior conditional order; one manual run executed; TRUE branch / Telegram arrived. Classification: INCONCLUSIVE / likely new-key send due to latest-done drift, NOT a confirmed idempotency bug. D-0193-A consumed.  
**Pinned validation outcome (batch 0199–0203, 2026-05-14):** D-0197-A = 1 decided; one manual pinned run on duplicate TEST-only workflow with override node; override output correct for 0193; `Load notification state` did not find existing 0193 row; `Store notification state` wrote task 0198 (not 0193). Classification: NOT SUCCESSFUL / partial pinning insufficient — downstream nodes reference dynamic upstream outputs by name. D-0197-A consumed.  
**Fully-pinned harness requirement (§10 clarification):** Same-key validation requires a fully-pinned harness with all dynamic upstream nodes removed/disabled and all downstream nodes using `$json.*` only. Design: `docs/automation/telegram-fully-pinned-validation-harness-design.md`. **D-0202-A pending** (controlled inspection/repair, no Execute).
