...
**Retry outcome (batch 0194–0198, 2026-05-14):** D-0193-A applied (= 1) per the user's prior conditional order; one manual run executed; TRUE branch / Telegram arrived / `Store notification state` executed. Classification: INCONCLUSIVE / likely new-key send due to latest-done drift; NOT a confirmed idempotency bug. D-0193-A is consumed.  
**Pinned validation outcome (batch 0199–0203, 2026-05-14):** D-0197-A = 1 decided; one manual pinned run executed on duplicate TEST-only workflow with override node; override node output correct for task 0193; but `Load notification state` did not find existing 0193 row; `Decide send or skip` routed TRUE; `Store notification state` wrote task 0198 (not 0193). Classification: NOT SUCCESSFUL / partial pinning insufficient — downstream nodes reference dynamic upstream outputs by name instead of `$json.*`. NOT a confirmed idempotency bug — harness failure prevents conclusions. D-0197-A consumed.  
**Fully-pinned harness requirement:** Same-key duplicate-skip validation requires a fully-pinned harness where all dynamic upstream nodes are removed/disabled and all downstream nodes use `$json.*` current item fields only. Design: `docs/automation/telegram-fully-pinned-validation-harness-design.md`. **D-0202-A pending** (controlled inspection/repair of TEST-only pinned workflow, no Execute).
