...
**Retry outcome (batch 0194–0198, 2026-05-14):** D-0193-A applied (= 1) per the user's prior conditional order; one manual run executed; TRUE branch / Telegram arrived / `Store notification state` executed. Classification: INCONCLUSIVE / likely new-key send due to latest-done drift; NOT a confirmed idempotency bug. D-0193-A is consumed. **D-0197-A pending** (one pinned-file duplicate-skip validation run). Same-key dynamic-input validation is structurally unstable — see `docs/tasks/done/0195-document-latest-done-drift-root-cause.md`.  
**Pinned-file requirement:** Same-key duplicate-skip validation must use a pinned `(done_file_path, done_file_sha)` chosen explicitly by the operator, per `docs/automation/telegram-pinned-file-duplicate-skip-validation-design.md`. Dynamic "Pick latest done file" cannot reliably reach the FALSE branch when docs-only batches keep adding newer done files.
