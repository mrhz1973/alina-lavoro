...
**Derived memory (Level 2) — last updated: 2026-05-14 (task 0250)**
...
| Latest-done selection fix | ✅ **Fixed + manually validated** (batch 0246–0250, 2026-05-14) — `Pick latest done file` was selecting stale task 0232; fixed via numeric sort descending; manual validation: Telegram 0245 arrived; **first scheduled tick post-fix pending observation** — not declared stable-after-fix yet | `docs/automation/telegram-mode-a-latest-done-selection-fix.md` |
| Post-activation monitoring | `docs/automation/telegram-mode-a-post-activation-monitoring-checklist.md` (updated with latest-done selection diagnostic, 2026-05-14) |
| Next step | Observe first scheduled tick post-fix: expect duplicate-skip for 0245 (no Telegram); if succeeds → Mode A confirmed stable-after-fix; if fails or duplicates → disable and record incident |
