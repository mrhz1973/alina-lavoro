...
Ultimo aggiornamento: 2026-05-12 — **Task 0115 completato** (tipo `n8n-runtime-activation`, 2026-05-12): Schedule Trigger aggiunto direttamente al queue reader `TEST - GitHub list Alina task queue` (5 min, Europe/Berlin); nodo Execute Workflow inverso spurio trovato e rimosso; test manuale pre-pubblicazione OK; workflow pubblicato come `queue-reader-schedule-5min` e attivato; primo tick automatico `has_task:false` silenzioso, nessuna scrittura GitHub; Manual Trigger e "When Executed by Another Workflow" invariati; nessun runner automatico; nessuna modifica app. Polling automatico ora operativo. Prossimo passo: task separato per validare ciclo `has_task:true` sotto gate manuale. App Alina **V1.9.2** stabile; `APP_VERSION` **1.9.2**; Apps Script **clasp `@24`**; tag **`v1.9.2-stable`**. **Rollback immediato:** **`v1.9.1-stable`** / **`@23`**. **Test manuale utente su `/exec` @24:** **OK** (2026-05-10). Workflow **main-only**; **`aggio:win`** (`docs/COMMANDS.md`).
...
1. Prossimo passo: gate orchestratore esplicito per autorizzare esecuzione task 0122 (setup Claude Code CLI subscription/no-API). App Alina V1.9.2 stabile e non toccata.
