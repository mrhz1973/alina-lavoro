...
Ultimo aggiornamento: 2026-05-12 — **LLMS-first routing enforced** (docs-only, 2026-05-12): CLAUDE.md updated — PROJECT_STATE.md and CHECKPOINT.md removed from default session start reading list; mandatory read order: LLMS.md → wiki/current-state.md → wiki/token-efficiency.md → task file → task-specific canonicals; PROJECT_STATE.md = fallback/audit only; CHECKPOINT.md = restart context only; large-file warnings may remain until future physical compression task; no runtime/app/deploy/tag/rollback; session: `docs/sessions/2026-05-12-enforce-llms-first-routing.md`. Precedente: **Agent-facing rules normalized in English** (docs-only, 2026-05-12): `docs/AI_RULES.md` and `docs/WORKFLOW.md` converted to technical English; English agent-facing operational summary added to `docs/ORCHESTRATOR_RULES.md`; `docs/LLMS.md` and `docs/tasks/templates/cursor-prompt-default.md` minor updates; historical canonical docs not translated; no runtime/app/deploy/tag/rollback; session: `docs/sessions/2026-05-12-normalize-agent-facing-rules-english.md`. Precedente: **Language policy for agents introdotta** (docs-only, 2026-05-12): regola linguistica per agenti e modelli locali formalizzata in `docs/AI_RULES.md` (sezione "Language policy for agents"); sintesi in `docs/ORCHESTRATOR_RULES.md`, `docs/LLMS.md`, `docs/wiki/token-efficiency.md`; design rationale in `docs/automation/local-llm-wiki-token-efficiency-design.md` (sezione 14); nota classifier in `docs/automation/ollama-classifier-planner-feasibility-post-wiki.md` (sezione 12); regola: inglese tecnico per reasoning interno, prompt, JSON/YAML, classifier, wiki agent-facing; italiano per output finale all'utente; base empirica: qwen3:8b più preciso e meno verboso in inglese tecnico; nessun runtime, nessuna installazione Ollama, nessun modello scaricato, nessuna modifica app, nessun deploy/tag/rollback; sessione: `docs/sessions/2026-05-12-agent-language-policy-token-efficiency.md`. Precedente: **Decisione post-0133 registrata** (docs-only, 2026-05-12): target iniziale Ollama locale aggiornato da MacBook Pro M2 a **workstation Windows** (AMD Ryzen 9 3900X / 32 GB RAM / RTX 3060 12 GB VRAM); MacBook Pro M2 diventa opzione secondaria/fase futura; VPS IONOS invariato (solo n8n, non per LLM); futuro task 0134 riformulato come "Windows Ollama Local Preflight Install" (runtime-gated); nessun runtime eseguito, nessuna installazione Ollama; Gate 7 (Decision Packet + conferma manuale utente) ancora richiesto prima dell'apertura del task preflight; sessione: `docs/sessions/2026-05-12-ollama-target-windows-decision.md`; documento feasibility aggiornato: `docs/automation/ollama-classifier-planner-feasibility-post-wiki.md` (sezione 11 aggiunta). Prossimo passo: **Gate 7 → task 0134 Windows Ollama Local Preflight (runtime-gated)**. Precedente: **Task 0133 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Ollama Classifier/Planner Feasibility (Post-Wiki) — feasibility in `docs/automation/ollama-classifier-planner-feasibility-post-wiki.md`; raccomandazione PROCEDERE con preflight runtime-gated; MacBook Pro M2 come macchina candidata per primo test; Qwen 2.5 7B come modello iniziale; input compatto post-wiki (~6–10k chars vs 47k+ pre-wiki — riduzione ~80%); 7 gate espliciti pre-installazione (qualità/sicurezza/hardware/policy/isolamento/supervisione/conferma utente); fallback invariante: Fase A (triage manuale orchestratore) sempre disponibile; Decision Packet non emesso; prossimo step: Gate 7 (conferma manuale utente + Decision Packet) prima di aprire task 0134 preflight runtime-gated; nessun runtime/app/deploy/tag/rollback; sessione: `docs/sessions/2026-05-12-ollama-classifier-planner-feasibility-post-wiki.md`. Prossimo passo: **Gate 7 → task 0134 Ollama Local Preflight Install (runtime-gated)** — oppure Implementer Bridge Design se preflight è rinviato. Precedente: **Task 0132 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): LLM Wiki / Token Efficiency Design — strato memoria derivata AI-friendly; `docs/LLMS.md` creato (entry point compatto agenti); `docs/wiki/` creata (current-state.md, token-efficiency.md, README.md); `docs/automation/local-llm-wiki-token-efficiency-design.md` creato; `CLAUDE.md` aggiornato; gerarchia 4 livelli definita; principio invariante: wiki è memoria derivata, canonici vincono sempre; risparmio token stimato ~40–80% per sessione; piano futuro Ollama non-runtime documentato; Decision Packet non emesso; nessun runtime/app/deploy/tag/rollback; sessione: `docs/sessions/2026-05-12-llm-wiki-token-efficiency-design.md`. Prossimo passo (storico): **Ollama Classifier/Planner Feasibility** (riformulato post-wiki). Precedente: **Task 0131 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): n8n Decision Packet Generator Design definito in `docs/automation/n8n-decision-packet-generator-design.md`; DP Generator progettato come componente logico tra Auto-Aggio e INBOX; architettura `Auto-Aggio → DP Generator → INBOX`; 10 trigger logici, mapping 13 campi canonici, 8 regole filtro anti-rumore, gate permanenti obbligatori, 10 failure modes, confini docs-only vs runtime-gated, MVP documentale; Decision Packet: non emesso; nessun runtime/app/deploy/tag/rollback; `docs/INBOX.md` NON creato; sessione: `docs/sessions/2026-05-12-n8n-decision-packet-generator-design.md`. Prossimo passo (storico): **0132**. Precedente: **Task docs-only completato** (rimozione regole operative obsolete, 2026-05-12): rimosse sezioni **«Regola richiesta esplicita di `aggio`»** e **«Regola sequenza prompt / aggio»** da `docs/ORCHESTRATOR_RULES.md`; `aggio` ora solo comando volontario/fallback; nuova **«Regola output verso implementatore»**: riga MODALITÀ + unico blocco prompt + nessun testo esterno salvo gate reale; template `cursor-prompt-default.md` aggiornato con `MODALITÀ: AGENT / IMPLEMENTAZIONE`; propagato a `WORKFLOW.md`, `AI_RULES.md`, `COMMANDS.md`, `runbook.md`, `permissions.md`; nessun runtime/app/deploy/tag/rollback; sessione: `docs/sessions/2026-05-12-remove-obsolete-operational-rules.md`. App **V1.9.2** stabile su `main`, tag `v1.9.2-stable`, Apps Script `@24`. Precedente: **Task 0129 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Human Decision Inbox Design definito in `docs/automation/human-decision-inbox-design.md`; INBOX progettata come luogo unico file-based per le decisioni vere — complemento di Auto-Aggio; struttura raccomandata Opzione A: singolo file `docs/INBOX.md` con 4 sezioni stato (Pending/Decided/Deferred/Superseded); template Markdown ingloba 13 campi Decision Packet Format + 8 campi INBOX-specifici; campo `kind` invariato in posizione 2; convenzione risposta `D-NNNN-X = N` / `defer` / `skip` / `retry`; 9 regole anti-rumore; gate manuali permanenti preservati; regole sicurezza anti-dati-sensibili; **`docs/INBOX.md` NON creato** (riservato a task futuro mixed/runtime-gated); **nessun Decision Packet emesso** (Opzione A univocamente determinata dai vincoli); ordine roadmap successivo confermato: 0131 DP Generator → 0132 Ollama → 0133 Implementer Bridge; eventuali cambi d'ordine via Decision Packet; nessun runtime/installazione/CLI/login/API key; sessione: `docs/sessions/2026-05-12-human-decision-inbox-design.md`. Precedente: **Task 0130 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Auto-Aggio Design definito in `docs/automation/auto-aggio-design.md`; meccanismo a due livelli per ridurre/eliminare scrittura manuale "aggio" — livello immediato (disciplina lettura GitHub da ChatGPT, zero runtime, ~50–60%) e livello automatizzato (n8n runtime-gated futuro, ~70–80%); 8 stati, 10 failure modes, 10 regole anti-falso positivo, formato riepilogo automatico, regole notifica (silenzioso per standard, attivo per DP/errore/ambiguità/gate), gate manuali permanenti preservati; nessun Decision Packet emesso; decisione di origine D-0128-A=2; ordine roadmap successivo: 0129 INBOX → 0131 DP Generator → 0132 Ollama → 0133 Implementer Bridge; eventuali cambi d'ordine via Decision Packet; nessun runtime/installazione/CLI/login/API key; sessione: `docs/sessions/2026-05-12-auto-aggio-design.md`. Precedente: **Task 0128 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): design loop autonomo asincrono in `docs/automation/autonomous-low-touch-loop-design.md`; architettura `GitHub → n8n → classifier/planner → implementatore → Decision Packet → utente`; 3 livelli A/B/C; Decision Packet D-0128-A emesso e risolto con scelta utente=2 (Auto-Aggio prima). Precedente: **Task 0127 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Decision Packet Format definito in `docs/automation/decision-packet-format.md`; template canonico 13 campi, 4 esempi (automation/infra/meta/alina-feature), regole uso, compatibilità INBOX/auto-aggio; criterio permanente "micro-interazioni umane eliminate" integrato; regola lifecycle aggiunta a docs/ORCHESTRATOR_RULES.md (creazione task = queue + sessione; PROJECT_STATE/CHECKPOINT solo al completamento); low-touch loop inaugurato; nessun runtime/installazione/CLI/login/API key; sessione: `docs/sessions/2026-05-12-decision-packet-format.md`. Precedente: **Task 0126 completato** (tipo `runner-market-research-docs-only`, 2026-05-12): ricerca documentale soluzioni di mercato runner/agent low-cost su fonti ufficiali; decisione 0124 confermata e rafforzata; manuale-supervisionata come baseline; identificate 3 direzioni future — Cursor CLI post-reset, Ollama locale planner zero-API, architettura ibrida; API cinesi NON raccomandate anche per dati non sensibili; documentazione in `docs/automation/runner-market-options-low-cost-api-research.md`. Precedente: **Task 0125 completato** (tipo `runner-feasibility-docs-only`, 2026-05-12): valutazione documentale Codex CLI su fonti ufficiali OpenAI; risultato: Codex CLI non raccomandato nel breve termine come runner VPS headless no-API-key; headless possibile con workaround (device code beta, copia auth.json) ma token in chiaro su VPS; Codex CLI tecnicamente possibile come uso locale supervisionato ma non prioritario; nessun runtime, installazione, login, API key; documentazione in `docs/automation/codex-cli-feasibility-check.md`. Precedente: **Task 0124 completato** (tipo `runner-decision-docs-only`, 2026-05-12): decisione Fase 3A no-API documentata; raccomandazione: breve termine modalità manuale-supervisionata — Claude Code locale principale, Windsurf/Cascade riserva, n8n solo queue reader; Claude Code CLI VPS non raccomandato finché resta vincolo no-API-key; documentazione in `docs/automation/runner-alternatives-no-api-decision.md`. Precedente: **Task 0123 completato** (tipo `cli-auth-planning-gated`, 2026-05-12): login/subscription Claude Code CLI su VPS headless bloccato. Precedente: **Task 0122 completato** (tipo `cli-setup-planning-gated`, 2026-05-12): Claude Code CLI 2.1.139 installata su VPS. App Alina **V1.9.2** stabile; `APP_VERSION` **1.9.2**; Apps Script **clasp `@24`**; tag **`v1.9.2-stable`**. **Rollback immediato:** **`v1.9.1-stable`** / **`@23`**. **Test manuale utente su `/exec` @24:** **OK** (2026-05-10). Workflow **main-only**; **`aggio:win`** (`docs/COMMANDS.md`).
...
**Task 0125 completato** (tipo `runner-feasibility-docs-only`, 2026-05-12): valutazione documentale Codex CLI eseguita; risultato: Codex CLI non raccomandato per runner VPS no-API-key nel breve termine — OpenAI raccomanda API key per automazione (viola presupposto); headless possibile con workaround (device code beta, copia auth.json) ma token in chiaro su VPS; nessun runtime, installazione, login, API key, VPS, n8n, runner automatico; decisione 0124 confermata; documentazione in `docs/automation/codex-cli-feasibility-check.md`; sessione: `docs/sessions/2026-05-12-codex-cli-feasibility-check.md`.

**Task 0126 completato** (tipo `runner-market-research-docs-only`, 2026-05-12): ricerca documentale soluzioni di mercato runner/agent low-cost su fonti ufficiali (Anthropic, OpenAI, GitHub Actions, Ollama, DeepSeek, Alibaba Cloud Model Studio, Moonshot/Kimi, ZhipuAI/GLM); decisione 0124 confermata e rafforzata; manuale-supervisionata come baseline; 3 direzioni future identificate — Cursor CLI post-reset, Ollama locale planner zero-API, architettura ibrida; API cinesi NON raccomandate anche per dati non sensibili; GitHub Actions fuori scope; runner automatico fuori scope; micro-task proponibili 0127–0130; nessun runtime, installazione, CLI, login, API key; documentazione in `docs/automation/runner-market-options-low-cost-api-research.md`; sessione: `docs/sessions/2026-05-12-runner-market-options-low-cost-api-research.md`.

**Task 0127 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Decision Packet Format definito in `docs/automation/decision-packet-format.md`; template canonico 13 campi, 4 esempi (automation/infra/meta/alina-feature), regole uso, compatibilità INBOX/auto-aggio; criterio permanente "micro-interazioni umane eliminate" integrato; regola lifecycle aggiunta a docs/ORCHESTRATOR_RULES.md (creazione task = queue + sessione; PROJECT_STATE/CHECKPOINT solo al completamento); nessun runtime/installazione/CLI/login/API key; sessione: `docs/sessions/2026-05-12-decision-packet-format.md`.
...
**Decisione D-0128-A registrata** (2026-05-12): risposta utente = **2** (Auto-Aggio prima). Decisione applicata e completata con task 0130.

**Task 0130 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Auto-Aggio Design definito in `docs/automation/auto-aggio-design.md`; meccanismo a due livelli — livello immediato (disciplina lettura GitHub da ChatGPT, zero runtime, riduzione ~50–60%) e livello automatizzato (n8n runtime-gated futuro, ~70–80%); 8 stati definiti, 10 failure modes, 10 regole anti-falso positivo, formato riepilogo automatico, notifiche silenzioso/attivo, compatibilità con Decision Packet Format e INBOX.md futura, gate manuali permanenti preservati; nessun Decision Packet emesso; nessun runtime/installazione/CLI/login/API key/VPS/n8n runtime/GitHub Actions; sessione: `docs/sessions/2026-05-12-auto-aggio-design.md`.

**Task 0129 completato** (tipo `low-touch-loop-docs-only`, 2026-05-12): Human Decision Inbox Design definito in `docs/automation/human-decision-inbox-design.md`; INBOX progettata come luogo unico file-based per le decisioni vere — complemento di Auto-Aggio; struttura raccomandata Opzione A: singolo file `docs/INBOX.md` con 4 sezioni stato (Pending/Decided/Deferred/Superseded); template Markdown ingloba 13 campi Decision Packet Format + 8 campi INBOX-specifici; campo `kind` invariato in posizione 2; convenzione risposta utente `D-NNNN-X = N` / `defer` / `skip` / `retry`; 9 regole anti-rumore; compatibilità con DP Format, Auto-Aggio, n8n futuro; gate manuali permanenti preservati; regole sicurezza anti-dati-sensibili; **`docs/INBOX.md` NON creato** in questo task (riservato a task futuro mixed/runtime-gated); **nessun Decision Packet emesso** (Opzione A univocamente determinata dai vincoli); nessun runtime/installazione/CLI/login/API key/VPS/n8n runtime/GitHub Actions; sessione: `docs/sessions/2026-05-12-human-decision-inbox-design.md`.

**Struttura raccomandata futura INBOX:** **Opzione A** — singolo file `docs/INBOX.md` con 4 sezioni stato. Creazione effettiva riservata a task futuro mixed/runtime-gated con gate manuale leggero.

**MVP raccomandato INBOX (3 fasi):** (1) ora solo design; (2) task futuro mixed/runtime-gated crea `docs/INBOX.md`; (3) task runtime-gated successivo integra n8n.

**Ordine roadmap successivo (confermato):** **0131** n8n Decision Packet Generator Design → **0132** Ollama Classifier/Planner Feasibility → **0133** Cursor/Implementer Bridge Design. Eventuali cambi d'ordine vanno proposti tramite Decision Packet (kind: `meta` o `automation`), non come commenti liberi.

1. Prossimo passo naturale: **0131 n8n Decision Packet Generator Design** (non creare in questo task). Auto-Aggio livello immediato + lettura disciplinata INBOX (quando creata in futuro) applicabili subito come disciplina ChatGPT. App Alina V1.9.2 stabile e non toccata. Nessun runtime, installazione, CLI, login, API key, VPS, n8n runtime, GitHub Actions, app, deploy, tag, rollback o runner automatico.
