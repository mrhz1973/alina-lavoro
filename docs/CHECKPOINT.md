...
Ultimo aggiornamento: 2026-05-12 — **Task 0114 bloccato** (publish blocker n8n: watcher non pubblicabile perché il sub-workflow queue reader risulta non pubblicabile — "no trigger nodes that require publishing"). Schedule Trigger aggiunto al watcher e test manuale OK, ma polling automatico non attivo. Prossimo passo: scelta architetturale per sbloccare 0114 (Opzione A/B/C — vedi CHECKPOINT Prossimo passo e sessione `docs/sessions/2026-05-12-n8n-watcher-schedule-trigger-publish-blocked.md`). App Alina **V1.9.2** stabile; `APP_VERSION` **1.9.2**; Apps Script **clasp `@24`**; tag **`v1.9.2-stable`**. **Rollback immediato:** **`v1.9.1-stable`** / **`@23`**. **Test manuale utente su `/exec` @24:** **OK** (2026-05-10). Workflow **main-only**; **`aggio:win`** (`docs/COMMANDS.md`).
