...
function parseSpreadsheetId_(urlOrId) {
  ...
  var txt = String(urlOrId).trim();
  var m = txt.match(/\/spreadsheets\/d\/([a-zA-Z0-9_-]+)/);
  if (m) return m[1];
  if (/^[a-zA-Z0-9_-]{20,}$/.test(txt)) return txt;
  return null;
}
...
function previewImportFromSpreadsheet(accessCode, urlOrId, sheetName, dataType) {
  ...
  if (!urlOrId) {
    return { success: false, message: 'URL o ID del Google Sheet mancante' };
  }

  var ssId = parseSpreadsheetId_(urlOrId);
  if (!ssId) {
    return { success: false, message: 'URL o ID non riconosciuto. Incolla l\'URL completo del Google Sheet o solo l\'ID (stringa di 44 caratteri).' };
  }
  ...
  try {
    ...
    return {
      success: false,
      message: 'Impossibile aprire il Google Sheet. Verifica che sia accessibile con il tuo account Google. Errore: ' + err.message
    };
  }
  ...
  if (!sheetName) {
    ...
    return {
      success: false,
      message: 'Specifica il nome del foglio (tab). Tab disponibili: ' + names.join(', ')
    };
  }
  ...
  if (!sh) {
    ...
    return {
      success: false,
      message: 'Foglio "' + sheetName + '" non trovato. Tab disponibili: ' + names.join(', ')
    };
  }

  if (sh.getLastRow() < 2) {
    return { success: false, message: 'Foglio "' + sheetName + '" senza dati (servono intestazioni + righe dati)' };
  }
  ...
  var rows = values
    ...
    .map(function(row) {
      var obj = {};
      headers.forEach(function(header, idx) {
        if (header) obj[header] = normalizeCell_(row[idx]);
      });
      return obj;
    });

  return buildImportPreview_({ headers: headers, rows: rows }, dataType || null, 'external-sheet:' + sheetName);
}

function applyReplaceFromExternalSheet(accessCode, urlOrId, sheetName, confirmed) {
  setupAlinaLavoro();
  requireAccess_(accessCode);

  if (!confirmed) {
    return { success: false, message: 'Conferma obbligatoria mancante' };
  }

  var ssId = parseSpreadsheetId_(urlOrId);
  if (!ssId) {
    return { success: false, message: 'URL o ID non riconosciuto' };
  }
  ...
  try {
    ...
    return { success: false, message: 'Impossibile aprire il Google Sheet: ' + err.message };
  }
  ...
  var sh = ss.getSheetByName(sheetName);
  if (!sh) {
    return { success: false, message: 'Foglio "' + sheetName + '" non trovato' };
  }

  if (sh.getLastRow() < 2) {
    return { success: false, message: 'Foglio "' + sheetName + '" senza dati' };
  }
  ...
  var peek = buildImportPreview_({ headers: headers, rows: rows }, null, 'ext');
  if (!peek.success) {
    return { success: false, message: 'Dati non riconosciuti: ' + (peek.message || 'tipo non rilevato') };
  }

  var dataType = peek.dataType;

  var normalized = rows.map(function(row) {
    return normalizeImportRow_(row, dataType);
  });
  var allValid = normalized.filter(function(r) {
    return validateRow_(r, dataType).length === 0;
  });

  if (allValid.length === 0) {
    return { success: false, message: 'Nessuna riga valida nel foglio esterno' };
  }

  var targetSheet = sheetNameForType_(dataType);
  ...
  try {
    try {
      backupSheet_(targetSheet);
    } catch (err) {
      return { success: false, message: 'Backup fallito (operazione annullata): ' + err.message };
    }

    var localSs = getSpreadsheet_();
    var localSh = localSs.getSheetByName(targetSheet);
    if (localSh && localSh.getLastRow() > 1) {
      localSh.getRange(2, 1, localSh.getLastRow() - 1, Math.max(localSh.getLastColumn(), 1)).clearContent();
    }
    ...
    allValid.forEach(function(row) {
      try {
        var prepared = prepareRowForWrite_(row, dataType, null);
        writeRowForType_(dataType, prepared);
        inserted++;
      } catch (err) {
        failed++;
    ...
    SpreadsheetApp.flush();
    log_('applyReplaceFromExternalSheet', 'ok', JSON.stringify({ dataType: dataType, inserted: inserted, failed: failed }));
